...
def load_sample(video_id, split_dir):
    # (Giữ nguyên 100% code load_sample ... )
    # Load manual poses
...
def compute_normalization_stats(data_dir, split='train'):
    # (Giữ nguyên 100% code compute_normalization_stats ... )
...
# ==========================================================
# NỘI DUNG TỪ dataset.py (Đã sửa cho mCLIP)
# ==========================================================

class SignLanguageDataset(Dataset):
    """
    Dataset cho Text → Pose (Đã cập nhật cho mCLIP)
    """
    ...
    def __init__(
        ...
        text_file=None,
        max_seq_len=120,
        max_text_len=64, # mCLIP thường dùng 77, nhưng 64 vẫn ổn
        normalize=True,
        stats_path=None
    ):
        ...
        # === THAY ĐỔI: Dùng AutoTokenizer của mCLIP ===
        self.mclip_name = "M-CLIP/XLM-Roberta-Large-Vit-L-14"
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(self.mclip_name)
        except Exception as e:
            print(f"Lỗi khi tải tokenizer: {e}")
            print("Vui lòng đảm bảo đã cài: !pip install -U sentence-transformers transformers")
            raise e
        
        # (Load video IDs và texts, giữ nguyên)
        ...
        print(f"✅ Using Tokenizer: {self.mclip_name}")
    
    def __len__(self):
        return len(self.video_ids)
    
    def __getitem__(self, idx):
        # (Load pose, normalize, pad... giữ nguyên)
        ...
        # === THAY ĐỔI: Tokenize bằng tokenizer mCLIP ===
    ...
    # (get_mean_std, collate_fn: Giữ nguyên 100%)
