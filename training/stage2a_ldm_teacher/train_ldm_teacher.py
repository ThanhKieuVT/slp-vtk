...
# === PHIÊN BẢN FINAL FIXED: AUTO-SCALING + MSE LOSS ===
...
# from models.losses import VelocityLoss # Tạm tắt để tập trung học Denoise
...
# Hàm tự động tính Scale Factor cho Latent
def estimate_scale_factor(autoencoder, dataloader, device, num_batches=10):
    print(f"⏳ Đang tính toán Latent Scale Factor dựa trên {num_batches} batches đầu tiên...")
    autoencoder.eval()
    all_latents = []
    ...
    print(f"✅ Scale Factor được chọn: {scale_factor:.6f}")
    print("-> Latent sau khi scale sẽ có std ~ 1.0 (Chuẩn cho LDM)")
    return scale_factor
...
def train_epoch(ldm_model, autoencoder, text_encoder, dataloader,
    ...
    for i, batch in enumerate(pbar):
        ...
        # 1. Encode Poses -> Latent
        with torch.no_grad():
            gt_z0 = autoencoder.encode(poses, mask=pose_mask)
            # QUAN TRỌNG: Scale latent về phân phối chuẩn
...
def main():
    ...
    # Tính toán scale factor dựa trên dữ liệu train thực tế
    ...
    noise_scheduler.config.prediction_type = "epsilon" # Predict Noise
    ...
    if args.resume_from and os.path.exists(args.resume_from):
        ...
        # Lấy scale factor cũ nếu có, nếu không dùng cái mới tính
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        print(f"Val   MSE Loss: {val_loss:.6f} (Best: {best_val_loss:.6f})")
        ...
        checkpoint = {
            ...
            'latent_scale_factor': latent_scale_factor, # Save scale factor để dùng khi Inference
            'args': args
        }
